...
<script type="text/html" data-template-name="model">
    ...
    <div class="form-row">
        <label for="node-input-inputMode" style="width: 160px;"><i class="fa fa-crop"></i> Input Mode</label>
        <select id="node-input-inputMode" style="width: 50%;">
            <option value="full_frame">Full Frame</option>
            <option value="cropped_roi">Cropped ROI</option>
        </select>
    </div>
    <div class="form-row" id="crop-size-row" style="display:none;">
        <label for="node-input-cropSize" style="width: 160px;"><i class="fa fa-expand"></i> Crop Size</label>
        <input type="text" id="node-input-cropSize" placeholder="112,112" style="width: 50%;">
    </div>
    <div class="form-row">
    ...
    <!-- Lua Script -->
    <div class="form-row">
        <label for="node-input-script"><i class="fa fa-file-code-o"></i> Lua Script</label>
        <div style="display:inline-flex; align-items:center; gap:6px; width:70%;">
            <label for="upload-lua-file" class="upload-button">
                <i class="fa fa-upload"></i> Upload
            </label>
            <input type="file" id="upload-lua-file" accept=".lua" style="display:none;">
            <input type="text" id="node-input-script" placeholder="/userdata/scripts/yolo11_tensor_detector.lua" style="flex:1; min-width:0;">
        </div>
    </div>
...
<script type="text/javascript">
    ...
    RED.nodes.registerType("model", {
        ...
        oneditprepare: function () {
            ...
            const inputModeSelect = document.getElementById("node-input-inputMode");
            const cropSizeRow = document.getElementById("crop-size-row");
            const cropSizeInput = document.getElementById("node-input-cropSize");

            function updateInputModeVisibility() {
                cropSizeRow.style.display = inputModeSelect.value === "cropped_roi" ? "flex" : "none";
            }
            inputModeSelect.addEventListener("change", updateInputModeVisibility);
            ...
            function fetchModelInfo() {
                ...
                fetch(`http://${host}:${port}/api/deviceMgr/getModelInfo`)
                    ...
                    .then((data) => {
                        // Only update display fields — do NOT overwrite node.modelInfo
                        // which stores per-node model info used by oneditsave.
                        const model = JSON.parse(data.data.model_info);
                        // Only auto-fill model name for unconfigured nodes
                        if (!uriInput.value) {
                            modelInput.value = model.model_name;
                        }
                        // Only auto-fill classes when the node has no custom classes.
                        const currentClasses = classesInput.value.trim();
                        if (!currentClasses) {
            ...
            // Only fetch global model info for unconfigured nodes.
            // Configured nodes already have their own per-node modelInfo.
            if (!uriInput.value && !modelInput.value) {
                fetchModelInfo();
            }
            ...
            const uploadLuaFile = document.getElementById("upload-lua-file");
            const scriptInput = document.getElementById("node-input-script");
            uploadLuaFile.addEventListener("change", function () {
                const file = this.files[0];
                if (file) {
                    node.scriptFile = file;
                    // Don't overwrite scriptInput with just filename -
                    // the actual device path will be set after upload succeeds (oneditsave)
                    RED.nodes.dirty(true);
                }
            });
                        ...
                        if (data.code === 0) {
                            ...
                            modelList.forEach((model) => {
                                ...
                                modelLink.addEventListener("click", function () {
                                    ...
                                    document.getElementById("node-input-script").value = model.script || "";
        ...
        oneditsave: function () {
            ...
            // Upload Lua script if a new file was selected
            const uploadLuaFile = document.getElementById("upload-lua-file");
            if (uploadLuaFile && uploadLuaFile.files.length > 0) {
                const luaFile = uploadLuaFile.files[0];
                const scriptInput = document.getElementById("node-input-script");
                // Set the target device path immediately so MQTT config has the full path
                // even if the upload is still in progress
                const targetPath = "/userdata/scripts/" + luaFile.name;
                if (scriptInput) scriptInput.value = targetPath;

                const formData = new FormData();
                formData.append("lua_file", luaFile);
                fetch(`http://${host}:${port}/api/deviceMgr/uploadLuaScript`, {
                    method: "POST",
                    body: formData,
                })
                    .then((res) => {
                        if (!res.ok) throw new Error("Upload API not available");
                        return res.json();
                    })
                    .then((result) => {
                        if (result.code === 0 && result.data && result.data.path) {
                            console.log("Lua script uploaded:", result.data.path);
                        }
                    })
                    .catch((err) => {
                        console.log("Lua upload skipped:", err.message);
                    });
            }
        },
    });

    // Intercept deploy: ensure at most one model node is wired to a preview node
    (function () {
        const _originalDeploy = RED.deploy.deploy.bind(RED.deploy);
        RED.deploy.deploy = function () {
            const modelNodes = RED.nodes.filterNodes({ type: "model" });
            const modelsWithPreview = modelNodes.filter(function (m) {
                return (m.wires || []).some(function (ws) {
                    return ws.some(function (wid) {
                        const n = RED.nodes.node(wid);
                        return n && n.type === "preview" && !n.d;
                    });
                });
            });
            if (modelsWithPreview.length > 1) {
                RED.notify(
                    "Only one model node can be connected to a preview node. " +
                    "Please remove the extra preview connection before deploying.",
                    "error"
                );
                return;
            }
            _originalDeploy.apply(this, arguments);
        };
    })();
</script>
