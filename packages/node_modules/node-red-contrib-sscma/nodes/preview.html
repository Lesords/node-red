<script type="text/javascript">
    ...
    (function () {
                ...
                if (port === 8080) {
                    ...
                    if (!$video) {
                        ...
                        if (!fo) {
                            ...
                            const vh = vb[3] || 360;
                            const vy = vb[1] || 0;
                            const headerOffset = 70; // Increase offset to match image position
                ...
                ws.addEventListener("message", function (event) {
                    try {
                        ...
                        if (obj && obj.hasOwnProperty("data")) {
                            ...
                            // Update node status with counts if available
                            updateNodeStatusWithCounts(previewId, data);
        ...
        // Find the upstream model node ID for a preview node
        function getUpstreamModelId(previewId) {
            try {
                const nodeSet = RED.nodes.createCompleteNodeSet(true);
                const preview = nodeSet.find((n) => n && n.id === previewId);
                if (!preview) return null;
                const tabId = preview.z;

                function findModel(targetId, visited = new Set()) {
                    if (visited.has(targetId)) return null;
                    visited.add(targetId);
                    for (const n of nodeSet) {
                        if (!n || !Array.isArray(n.wires)) continue;
                        if (tabId && n.z !== tabId) continue;
                        for (const arr of n.wires) {
                            if (Array.isArray(arr) && arr.includes(targetId)) {
                                if (n.type === "model" && !n.d) return n.id;
                                const found = findModel(n.id, visited);
        ...
        function determinePortForPreview(previewId) {
            ...
            // For model upstream: calculate port matching C++ side (8090 + model index)
            if (types.includes("model")) {
                const modelId = getUpstreamModelId(previewId);
                if (modelId) {
                    let modelIndex = 0;
                    try {
                        RED.nodes.eachNode(function (n) {
                            if (n.type === "model" && !n.d && n.id < modelId) {
                                modelIndex++;
                            }
                        });
                    } catch (e) {}
                    return 8090 + modelIndex;
                }
                return 8090;
            }
        ...
        function updateNodeStatusWithCounts(previewId, data) {
            try {
                if (data.counts && Array.isArray(data.counts) && data.counts.length >= 4) {
                    const countA = data.counts[0];
                    const countB = data.counts[1];
                    const countBA = data.counts[2];
                    const countAB = data.counts[3];
                    const textContent = `A: ${countA} B: ${countB} A->B: ${countAB} B->A: ${countBA}`;
                    
                    // Create or update a text element in the SVG to show counts
                    const $container = document.getElementById(previewId);
                    if ($container) {
                        let $statusText = document.getElementById(`status-text-${previewId}`);
                        if (!$statusText) {
                            $statusText = createSVGElement("text", {
                                id: `status-text-${previewId}`,
                                x: "10",
                                y: "55", // Move down to avoid overlap with node
                ...
                console.error('Error updating node status with counts:', e);
            }
        }
        ...
        const render = function (id, data) {
            ...
            if (!$img) {
                $img = createSVGElement("image", {
                    ...
                    x: "0",
                    y: "70", // Move image down to make room for status text
                });
            ...
            if (!$group) {
                $group = createSVGElement("g", {
                    ...
                    transform: "translate(0, 70)", // Move group down to match image position
                });
            ...
            if (data.image) {
                ...
                // Scale image to match resolution coordinate space so boxes align correctly
                if (data?.resolution) {
